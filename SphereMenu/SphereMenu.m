...
@synthesize item,currentX,currentY;
- (instancetype)initWithStartPoint:(CGPoint)startPoint startImage:(UIImage *)startImage submenuImages:(NSArray *)images
{
    if (self = [super init]) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(positionPanItem:::) name:@"MyNotification" object:nil];
...
- (void)positionPanItem:(CGFloat)x :(CGFloat)y :(NSNotification *) notification{
//    NSLog(@"%f %f",self.center.x,self.center.y);
    NSNumber * pointX = notification.userInfo[@"pointX"];
    NSNumber * pointY = notification.userInfo[@"pointY"];
    //NSLog(@"%f %f",[pointX floatValue],[pointY floatValue]);
    CGFloat currentPosX = [pointX floatValue];
    CGFloat currentPosY = [pointY floatValue];
    currentX = currentPosX;
    currentY = currentPosY;
//    NSLog(@"%f %f",currentX,currentY);
    for (int i  = 0; i < self.count; i++) {
        UIImageView * item = self.items[i];
        item.center = CGPointMake(currentPosX, currentPosY);

    }
    //item.center = CGPointMake(currentX, currentY);
...
- (void)commonSetup
{
    ...
    // setup the items
    for (int i = 0; i < self.count; i++) {
        item = [[UIImageView alloc] initWithImage:self.images[i]];
...
- (CGPoint)centerForSphereAtIndex:(int)index
{
    ...
    //CGPoint startPoint = CGPointMake(currentX, currentY);
...
//    CGPoint position = [positionValue CGPointValue];
    CGPoint position = CGPointMake((currentX + ((index + 1)* 60)), currentY);
